...
| `judge-composition` **present but unexecutable** | Its schema selects qualified parameters from four registries whose enumeration lives in a design record that is not public, so a reader outside that repository can read the protocol and cannot fill the schema. **Treat this as the row above**: compose a judge seat from § *The players* — one blind to the prediction and the stake, scoring utility on the manipulated axis — and state that the panel is uncomposed and the output is a verdict. Do not invent registry parameters to fill a schema you cannot check. | Yes, with the downgrade stated. |
| `harness-traps` | The "a loaded skill is not an applied one" check goes unstated, so a returned `Skill` call can be mistaken for arrived guidance. | Yes — and check it by hand. |
